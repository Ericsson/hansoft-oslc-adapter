	...
	xmlhttp.onreadystatechange = function() {
		if (xmlhttp.readyState==4 && xmlhttp.status==200) {
			...
			for( var i=0; i<resp.results.length; i=i+1 ) {
				...
				item.text = decodeURI( title.replace(/\+/g, " ") ); 
...
/*	if (window.location.hash == '#oslc-core-windowName-1.0') {       
	...
	if( window.parent != null ) {
		window.parent.postMessage(oslcResponse, "*");
	} else {
		window.postMessage(oslcResponse, "*");
	}
...
function encodePass(baseUrl){
	xmlhttp = new XMLHttpRequest();
	plainPassword = document.getElementById("plainPassword").value;
	window.open(baseUrl + "?plainPassword=" + encodeURIComponent(plainPassword),"_self")
	xmlhttp.open("GET", baseUrl + "?plainPassword=" + encodeURIComponent(plainPassword), true);	
	xmlhttp.send();
}
